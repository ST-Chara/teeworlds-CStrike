...
IGameController::IGameController(class CGameContext *pGameServer)
{
	...
	m_IsEnd = false;
	m_PreparTimer = 0;
	m_BuyTimer = Server()->TickSpeed() * g_Config.m_SvBuyTime;
	m_GameFlags = 0;
...
void IGameController::PostReset()
{
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		{
			GameServer()->m_apPlayers[i]->m_Money = 0;
			if(GameServer()->m_apPlayers[i]->m_IsDead != TEAM_SPECTATORS)
			{
				GameServer()->m_apPlayers[i]->SetTeam(GameServer()->m_apPlayers[i]->m_IsDead,false);
				GameServer()->m_apPlayers[i]->m_IsDead = TEAM_SPECTATORS;
			}
			GameServer()->m_apPlayers[i]->Clear();
...
void IGameController::Tick()
{
	...
	// time for buy from shop
	if(m_BuyTimer) m_BuyTimer--;
}
...
void IGameController::Snap(int SnappingClient)
{
	...
	pGameInfoObj->m_RoundNum = g_Config.m_SvRoundlimit;
	pGameInfoObj->m_RoundCurrent = m_RoundCount < g_Config.m_SvRoundlimit ? m_RoundCount+1:m_RoundCount;
}
...
int IGameController::GetAutoTeam(int NotThisID)
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(GameServer()->m_apPlayers[i] && i != NotThisID)
		{
			...
			//edited
			if(GameServer()->m_apPlayers[i]->m_IsDead >= TEAM_RED && GameServer()->m_apPlayers[i]->m_IsDead <= TEAM_BLUE)
				aNumplayers[GameServer()->m_apPlayers[i]->m_IsDead]++;
		}
	}
...
bool IGameController::CheckTeamBalance()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(pP && pP->m_IsDead != TEAM_SPECTATORS)
		{
			aT[pP->m_IsDead]++;
		}
	}
	...
	if(m_GameOverTick == -1 && !m_Warmup && !GameServer()->m_World.m_ResetRequested)
	{
		if(IsTeamplay())
		{
			...
			if((g_Config.m_SvScorelimit > 0 && (m_aTeamscore[TEAM_RED] >= g_Config.m_SvScorelimit || m_aTeamscore[TEAM_BLUE] >= g_Config.m_SvScorelimit)) ||
				...
				(g_Config.m_SvRoundlimit > 0 && m_RoundCount >= g_Config.m_SvRoundlimit))
			{
...
void IGameController::ChangeRound()
{
	if(m_RoundCount < g_Config.m_SvRoundlimit)
	{
		if(!m_PreparTimer && !m_IsEnd)
		{
			m_RoundCount++;
			m_PreparTimer = Server()->TickSpeed() * 3;
		}
	}
}

bool IGameController::CheckRound()
{
	bool Red=false,Blue = false;
	bool sRed=false,sBlue = false;
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(GameServer()->m_apPlayers[i])
		{
			...
			if(GameServer()->m_apPlayers[i]->m_IsDead == TEAM_RED)
				sRed = true;
			if(GameServer()->m_apPlayers[i]->m_IsDead == TEAM_BLUE)
				sBlue = true;
	...
	else
	{
		if(!Red&!sRed || !Blue&!sBlue)
		{
			m_RoundCount = g_Config.m_SvRoundlimit;
			return true;
		}
	}
...
void IGameController::Update()
{
    ...
    if(m_PreparTimer && m_IsEnd)
    {
        if(m_PreparTimer % Server()->TickSpeed() == 0)
        {
            GameServer()->CreateSoundGlobal(SOUND_HOOK_NOATTACH);
            char aBuf[256];
                str_format(aBuf, sizeof(aBuf), "下一个挑战 : %d of %d\n%d秒后开始\n\n输入/shop查看商店，或者检查投票界面",
                           m_RoundCount+1, g_Config.m_SvRoundlimit, m_PreparTimer/Server()->TickSpeed());
        ...
        if(m_PreparTimer % Server()->TickSpeed() == 0)
        {
            char aBuf[256];
                str_format(aBuf, sizeof(aBuf), "\n \n \n \n下一个挑战 : %d 秒后",
                           m_PreparTimer/Server()->TickSpeed());
		...
		if(!m_PreparTimer)
		{
			...
			for(int i = 0; i < MAX_CLIENTS; i++)
			{
				if(GameServer()->m_apPlayers[i])
				{
					...
					GameServer()->m_apPlayers[i]->m_Money += g_Config.m_SvMoneyWinner;
					GameServer()->m_apPlayers[i]->KillCharacter();
				}
			}

			m_BuyTimer = Server()->TickSpeed() * g_Config.m_SvBuyTime;
			m_PreparTimer = Server()->TickSpeed() * 10;
		}
    }
}
